...
// ########## КОМПОНЕНТЫ
import { ImgProgress, TextGame, Price, BtnGameType, Badges, Popup, CardCase } from "@shared/ui";
...
import { isWithinDays } from "@shared/funcs/time";
...
const PreviewCase = ({ game, order }: IPreviewCase): JSX.Element => {
   ...
   const isNeu = useMemo(() => isWithinDays(game.dtCreated, 7), [game]);

   const vibrate = useVibrate();
   const navigate = useNavigate();

   const onInfoModal = useCallback(() => {
      vibrate.apply("soft");
      open(<>
         <Screen closeAction={close}>
            <Popup title={`правила игры`} style="info" closeAction={close} button={{ name: 'ОК', action: close, checkbox: "Больше не показывать." }}>
               <div></div>
            </Popup>
         </Screen>
      </>);
   ...
   const onPlay = useCallback(() => {
      vibrate.apply("soft");
      navigate(`/game/case/${game.slug}`);
   }, [vibrate, navigate, game]);

   const onCaseModal = useCallback(() => {
      vibrate.apply("soft");
      open(<>
         <Screen closeAction={close}>
            <Popup title={`кейс ${game.title}`} closeAction={close} button={{ name: 'ИГРАТЬ', action: onPlay }}>
               <CardCase game={game} />
            </Popup>
         </Screen>
      </>);
   }, [vibrate, game, onPlay, open, close]);

   return (
      <div className={`games-preview-item ${order ? `games-preview-item-order-${order}` : ''}`.trim()} data-style={game.style} data-game-id={game.id} data-position={game.position}>

         <div className="games-preview-case spins" onClick={onCaseModal}>
            ...
            <div className="game-preview-case-title">
               <div className="preview-case-text">
                  <TextGame type="line" content={game.title.toUpperCase()} extraClass="preview-case-topic" />
            ...
            <div className="game-preview-case-price">
               ...
               <Price value={game.price} justify="end" size={20} animation />
            </div>

         </div>

         <div className="preview-badges">
            {game.timer && <div className="preview-badges-item"><Badges.Timer dtStart={game.timer.dtStart} duration={game.timer.duration} style="timer" /></div>}
            {game.limit && <div className="preview-badges-item"><Badges.Limit total={game.limit.total} purchased={game.limit.purchased} style="limit" /></div>}
            {isNeu && <div className="preview-badges-item"><Badges.Text text="новый" style="new" extraClass="_shimmer" /></div>}
            <div className="preview-badges-item"><BtnGameType type="case" action={onInfoModal} /></div>
         </div>
